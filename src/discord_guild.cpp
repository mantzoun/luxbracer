...
namespace luxbracer {
    ...
    int DiscordGuild::channel_add(const dpp::channel & channel) {
        return verify_and_add_channel(DiscordChannel(channel.id, channel.parent_id, channel.name));
    }

    int DiscordGuild::channel_add(const dpp::channel_create_t & channel) {
        return verify_and_add_channel(DiscordChannel(channel.created.id, channel.created.parent_id, channel.created.name));
    }

    int DiscordGuild::channel_update(const dpp::channel_update_t & channel) {
        return verify_and_update_channel(DiscordChannel(channel.updated.id, channel.updated.parent_id, channel.updated.name));
    }

    int DiscordGuild::channel_delete(const dpp::channel_delete_t & channel) {
        return verify_and_delete_channel(DiscordChannel(channel.deleted.id, channel.deleted.parent_id, channel.deleted.name));
    }

    std::list<DiscordChannel *> DiscordGuild::channel_get(const std::string& name, dpp::snowflake channel_id, dpp::snowflake parent_id) {
