...
engineError Engine::systemAdd(const System & system) {
  if ( system_map.count(system.getName()) != 0 ) {
    logger->warn("System " + system.getName() + " exists");
    return ENGINE_ITEM_EXISTS;
  }

  logger->debug("System " + system.getName() + "(" +
                std::to_string(system.getId()) + ") added");
...
engineError Engine::planetAdd(const std::string & name,
...
Planet * Engine::planetGet(const std::string & name,
                           const std::string & system) {
  System * s = systemGet(system);
...
engineError Engine::planetRemove(const std::string & name,
                                 const std::string & system) {
  System * s = systemGet(system);
